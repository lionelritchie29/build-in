...
export default function StepTwo({ type, setActiveIdx }: Props) {
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      if (custom) {
        setValue('userNote', custom.consultation?.designerNote ?? '');
        setValue('designerNote', custom.consultation?.userNote ?? '');
      }
    }
  }, [setValue]);
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className='space-y-3 mt-4'>
      <h1 className='text-center font-semibold text-2xl'>Konsultasi</h1>
